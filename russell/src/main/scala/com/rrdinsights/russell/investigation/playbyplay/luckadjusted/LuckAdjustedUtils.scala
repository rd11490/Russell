...
import com.rrdinsights.russell.utils.{NullSafe, TimeUtils}
...
object LuckAdjustedUtils {
  ...
  def determinePossession(events: Seq[(PlayByPlayWithLineup, Option[ScoredShot])]): Integer = {
  ...
  def countPoints(events: Seq[(PlayByPlayWithLineup, Option[ScoredShot])], freeThrowMap: Map[Integer, jl.Double]): Seq[TeamPoints] = {
    events.filter(v => isShotOrFreeThrow(v))
  ...
  private def isShotOrFreeThrow(event: (PlayByPlayWithLineup, Option[ScoredShot])): Boolean =
  ...
  def buildStintFromShot(event: (PlayByPlayWithLineup, Option[ScoredShot])): TeamPoints = {
    try {
      val teamId = event._1.player1TeamId
      event._2.map(shot => {
        val points = shot.shotValue.toDouble * shot.shotMade
        val expectedPoints = if (isThreePointShot(shot)) shot.expectedPoints.doubleValue() else points
        val fieldGoals = if (isMadeShot(shot)) 1 else 0
        val threePtAttempts = if (isThreePointShot(shot)) 1 else 0
        val threePtMade = if (isThreePointShot(shot) && isMadeShot(shot)) 1 else 0
        TeamPoints(
          teamId,
          points,
          expectedPoints,
  ...
  def buildStintFromFreeThrow(event: (PlayByPlayWithLineup, Option[ScoredShot]), freeThrowMap: Map[Integer, jl.Double]): TeamPoints = {
    val teamId = event._1.player1TeamId
    ...
    TeamPoints(teamId, points, expectedPoints, freeThrowAttempts = 1, freeThrowsMade = points.intValue())
  }
  ...
  def seperatePossessions(events: Seq[(PlayByPlayWithLineup, Option[ScoredShot])]): Seq[(Seq[(PlayByPlayWithLineup, Option[ScoredShot])], Integer)] = {
    ...
    while (i < events.size) {
      ...
      if (isEndOfPossesion(event, possession)) {
        if (PlayByPlayUtils.isEndOfPeriod(event)) {
  ...
  def isEndOfPossesion(event: (PlayByPlayWithLineup, Option[ScoredShot]),
                       lastEvents: Seq[(PlayByPlayWithLineup, Option[ScoredShot])]): Boolean = {
    lastEvents.nonEmpty &&
      (PlayByPlayUtils.isMakeAndNotAndOne(event, lastEvents) ||
        PlayByPlayUtils.isLastEventDefensiveRebound(event, lastEvents) ||
        PlayByPlayUtils.isLastMadeFreeThrow(lastEvents) ||
        PlayByPlayUtils.isTurnover(lastEvents) ||
        PlayByPlayUtils.isSubstitutionNotDuringFreeThrows(event, lastEvents))
  ...
  def readPlayByPlay(where: String*): Seq[PlayByPlayWithLineup] =
    MySqlClient.selectFrom(NBATables.play_by_play_with_lineup, PlayByPlayWithLineup.apply, where: _*)
...
final case class MissingEventException(private val message: String = "") extends Exception(message)