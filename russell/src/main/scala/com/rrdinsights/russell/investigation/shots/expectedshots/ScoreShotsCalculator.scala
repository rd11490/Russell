...
import com.rrdinsights.russell.investigation.shots.{
...
object ScoreShotsCalculator {
  ...
  private def joinShotsWithShotCharts(
      shotsWithPlayers: Seq[((Integer, String), ShotWithPlayers)],
      shotCharts: Map[(Integer, String), PlayerShotChartSection])
    : Seq[(ShotWithPlayers, PlayerShotChartSection)] =
    shotsWithPlayers.map(
      v =>
        (v._2,
         shotCharts.getOrElse(v._1, emptyShotChartSection(v._1._1, v._1._2))))

  private def emptyShotChartSection(playerId: Integer,
                                    bin: String): PlayerShotChartSection = {
    println(s"$playerId - $bin")
    PlayerShotChartSection(s"${playerId}_$bin",
                           playerId,
                           bin,
  ...
  private def readShotsWithPlayers(where: String*): Seq[ShotWithPlayers] =
    MySqlClient.selectFrom(NBATables.lineup_shots,
                           ShotWithPlayers.apply,
                           where: _*)

  private def readShotCharts( /*IO*/ ): Seq[PlayerShotChartSection] =
    MySqlClient.selectFrom(NBATables.player_shot_charts,
                           PlayerShotChartSection.apply)
  ...
  private def toScoredShot(shotWithPlayers: ShotWithPlayers,
                           shotChart: PlayerShotChartSection,
                           dt: String): ScoredShot = {
    ScoredShot(
      ...
      if (shotChart.shots.toDouble > 0.0) {
...
private object ScoredShotsArguments {
  ...
  def apply(args: Array[String]): ScoredShotsArguments =
    new ScoredShotsArguments(args)
