...
object BoxScoreSummaryDownloader {
  ...
  def downloadAndWriteAllBoxScoreSummaries(gameLogs: Seq[GameRecord], season: String, dt: String): Unit = {
    val boxscoreSummary = downloadAlBoxScoreSummaries(gameLogs)
    val out = boxscoreSummary.flatMap(_.gameSummary)
  ...
  private def downloadAlBoxScoreSummaries(gameLogs: Seq[GameRecord]): Seq[BoxScoreSummary] = {
    gameLogs
      .map(_.gameId)
      .distinct
