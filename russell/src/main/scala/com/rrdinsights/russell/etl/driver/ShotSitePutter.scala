...
object ShotSitePutter {
  ...
  def main(strings: Array[String]): Unit = {
    ...
    val where = Seq(s"season = '${args.season}'",s"seasonType = '${args.seasonType}'")
    ...
    if (args.transferPlayers) {
      val players = MySqlClient.selectFrom(NBATables.player_info, PlayerInfo.apply)

      players.grouped(100).foreach(ShotSiteClient.postPlayers)
    }

    if (args.transferTeams) {
      val teams = MySqlClient.selectFrom(NBATables.team_info, TeamInfo.apply)

      teams.grouped(100).foreach(ShotSiteClient.postTeams)
    }
...
private final class ShotSitePutterArguments private (args: Array[String])
    ...
    with SeasonOption  with RunAllOption {
  ...
  lazy val transferTeams: Boolean = has(ShotSitePutterArguments.TeamsOption) || runAll

  lazy val transferPlayers: Boolean = has(ShotSitePutterArguments.PlayersOption) || runAll

  lazy val transferShots: Boolean = has(ShotSitePutterArguments.ShotsOption) || runAll
...
private object ShotSitePutterArguments {
  ...
  val PlayersOption: cli.Option =
    new cli.Option(null, "players", false, "Transfer player info to the shot site")
