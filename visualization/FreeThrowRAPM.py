...
for season in seasons:
    ...
    def calc_ftr(row):
        ...
        row["FreeThrowPercent"] = ftm/fta

        return row


    stints = stints[stints["possessions"] > 0]
    stints = stints[stints["freeThrowAttempts"]>0]
    ...
    stintX_ftr, stintY_ftr, possessions_ftr = extract_stints(stints.copy(True), "FreeThrowPercent")
    ...
    results_ftr, ftr_intercept = calculate_rapm(stintY_ftr, stintX_ftr, possessions_ftr, lambdas_ftr, "RA_FT%",
                                 "FreeThrowPercent", stints.copy(True))
    ...
    merged.to_csv("results/Real Adjusted FT {}.csv".format(season))
