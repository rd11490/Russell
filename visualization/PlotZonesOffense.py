...
shotZonesData = pd.read_csv("data/ePPSZonedO.csv")

shotZonesData[valueForPlotting] = shotZonesData["pointsAvg"] - shotZonesData["expectedPointsAvg"]
...
for name in teamNames:
    ...
    plt.title("Offensive Points Per Shot for: {}".format(name))
    plt.colorbar(sm, ticks=[minVal, (maxVal+minVal)/2, maxVal], label='PPS - ePPS')
    plt.savefig("OShotCharts/{}_O".format(name), dpi=900)
    plt.close()
